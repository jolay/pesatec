...
//		  	exit;
	...
	function getDebtorGroup()
	{
		...
		if(is_array($debtorGroupHandles))
		{
			for($i=0;$i<count($debtorGroupHandles);$i++)
			{
				if (!$debtorGroupHandles[$i]->Number)
				{
					...
					$debtorGroupHandle->Number = $debtorGroupHandles[$i];
					$this->debtorGroupHandles = $debtorGroupHandle;
					return $debtorGroupHandle;
					break;
				}
				$this->debtorGroupHandles = $debtorGroupHandles[$i];
				return $debtorGroupHandles[$i];
			}
		}
		else if(is_object($debtorGroupHandles))
		{
			$this->debtorGroupHandles = $debtorGroupHandles;
...
//		$accountHandle->Number = 6;
		...
		try
		{
			...
			$this->client->CashBookEntry_SetText ( array ('cashBookEntryHandle' => $cashBookEntryHandle, 'value' => 'INV ('.$bookHandle->Number.') CUST ('.$d['name'].')' ) );
