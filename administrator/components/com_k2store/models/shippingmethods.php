 ...
 * @since 1.5
 */
class K2StoreModelShippingMethods extends JModel
{
	/**
	 *
	 * @var array
	...
	function _buildContentWhere()
	{
		...
		if ($search) {
			$where[] = 'LOWER(a.shipping_method_name) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false ).
			           'OR LOWER(a.shipping_method_type) LIKE '.$db->Quote( '%'.$db->getEscaped( $search, true ).'%', false );
		}
